  ...
  pieces: [
    {
      ...
      body: "Every Stripe webhook handler must verify the Stripe-Signature header with the endpoint secret using the raw request body, before parsing or trusting any field. Never act on an unverified event. Read the webhook secret from server config, not from the request, and return 400 on verification failure.",
      scopeType: "folder",
      priority: "high",
      enforcement: "strict",
    },
    {
      ...
      body: "Stripe delivers at least once, so handlers must be idempotent. Record processed event IDs and skip duplicates, or make the side effect itself idempotent (upsert by a stable key). Do slow work outside the request and return 2xx quickly so Stripe does not retry a successful delivery.",
      scopeType: "folder",
      priority: "high",
    ...
    },
    {
      ...
      body: "Prefer Stripe Checkout Sessions for subscriptions and standard one-time payments: it handles SCA, tax, and the payment UI for you. Use PaymentIntents with the Payment Element only when you need a fully custom in-app payment flow. Drive entitlements from webhook events, not from the client redirect result.",
    },
    {
      kind: "memory",
      ...
      body: "The Stripe secret key and webhook signing secret are read from server-side environment configuration only. Never expose them to the browser; only the publishable key is client-safe. In edge or serverless functions, verify the caller and the webhook signature before performing any billing action.",
    },
    {
      kind: "skill",
      nodePath: "/",
